...
export function blocksToMarkdown(blocks: BlockObjectResponse[]): string {
  return blocks
    .map((block) => {
      switch (block.type) {
        case "heading_1": return `# ${richTextToString(block.heading_1.rich_text)}`;
        case "heading_2": return `## ${richTextToString(block.heading_2.rich_text)}`;
        case "heading_3": return `### ${richTextToString(block.heading_3.rich_text)}`;
        case "paragraph": return richTextToString(block.paragraph.rich_text);
        case "bulleted_list_item": return `- ${richTextToString(block.bulleted_list_item.rich_text)}`;
        case "numbered_list_item": return `1. ${richTextToString(block.numbered_list_item.rich_text)}`;
        case "code": return `\`\`\`${block.code.language || ""}\n${richTextToString(block.code.rich_text)}\n\`\`\``;
        case "quote": return `> ${richTextToString(block.quote.rich_text)}`;
        case "divider": return "---";
        case "callout": return `> ${richTextToString(block.callout.rich_text)}`;
        default: return "";
      }
    })
    .filter(Boolean)
