...
本文件旨在說明如何將 HP 終端使用者工具包裝成安全的可發佈版本。整個流程包括整合、簽章與封裝等一系列步驟，由 eToken Server 作為協調者，與 Dongle Server (安全核心) 和 GLBin (檔案處理工具) 協同運作完成。

[未有直接 Source 錨點，待確認] 此架構圖展示了系統中各個元件的部署位置及其相互之間的通訊關係。


1. **WIP 階段**：將 Public Key 嵌入原始 Bin，依晶片規則重算 checksum、產生 WIP 檔。
2. **SIGN 階段**：計算 wip.bin 的 Hash，產生 Signed bin 檔。
3. **ROM 階段**：依晶片演算法，組裝最終檔案 rom。
...
本文件主要說明了 HP 終端使用者工具的打包流程，包括 WIP、SIGN 和 ROM 等階段的詳細步驟。文中引用了相關的技術說明文件，以提供更完整的背景資訊。整個流程由 eToken Server 協調，確保最終產品的安全性。