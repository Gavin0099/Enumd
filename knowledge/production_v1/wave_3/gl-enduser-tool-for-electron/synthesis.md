基於提供的內容邊界，以下是 'GL Enduser Tool for Electron' 的文件合成報告:

# GL Enduser Tool for Electron 文件報告

本專案旨在開發一個跨平台（Windows & macOS）的桌面應用程式，透過 [Electron](https://www.electronjs.org/) 框架提供統一的使用者介面。主要功能是與多款 USB Hub 硬體進行通訊，以實現韌體版本的查詢與線上更新。支援的硬體包括 Hub、Scaler 和 PD。

本專案採用 [Electron](https://www.electronjs.org/) 作為主要框架，並透過 [Node.js Native Addon](https://nodejs.org/api/addons.html) 技術橋接 JavaScript 與底層 C++/Objective-C 程式碼，以呼叫已分別封裝好的原生 SDK（Windows 為 `.dll`，macOS 為 `.dylib`）。

本專案為 Electron 跨平台桌面應用，整合 Native Addon 以支援 Windows 及 macOS 環境下的 USB Hub 韌體查詢與升級功能。以下整理各主要檔案及其功能、互動流程，以及架構圖。
...
1. 使用者啟動 App，`main.js` 建立主視窗並載入 UI (`index_2.html`)。
2. `preload.js` 將一組安全的 API (例如 `window.electron.getUsbHubList`) 注入到前端的 `window` 物件中。
3. 使用者在 UI 上操作，觸發 `renderer.js` 呼叫 `window.electron.getUsbHubList()`。
4. `preload.js` 接收到呼叫，立即透過 `ipcRenderer.invoke` 將請求與參數發送給 `main.js`。
5. `main.js` 中的 `ipcMain.handle` 監聽到請求，接著呼叫已載入的 `addon.node` 的對應函式。
6. `addon.node` 根據當前平台執行對應的程式碼：
   - Windows 執行 `addon.cpp` 並動態載入 `GL_SDK.dll`。
   - macOS 執行 `addon.mm` 並動態載入 `GL_SDK.dylib`。
7. 執行結果從原生函式庫逐層回傳：`DLL/dylib → Addon → main.js → renderer.js`。
8. `renderer.js` 收到最終結果後，更新 UI 介面進行顯示。
...
- [未有直接 Source 錨點，待確認] 若要新增/修改原生功能，需要同步修改：
