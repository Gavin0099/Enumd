...
exported_at: '2026-04-06T13:12:29.524Z'
is_summarized: false
relations:
  manual: []
  inferred:
...
```mermaid
%% eToken Signing Process Flow (Bit6=0) - Final Cleaned Version
flowchart TD
    %% --- Style Definitions (Outline Style for Max Readability) ---
    classDef file fill:transparent,stroke:#6B7280,stroke-width:2px
    classDef process fill:transparent,stroke:#4338CA,stroke-width:2px
    classDef condition fill:transparent,stroke:#DC2626,stroke-width:2.5px
    classDef config fill:transparent,stroke:#10B981,stroke-width:2px,stroke-dasharray: 5 5

    %% --- Stages ---
    subgraph S1 [階段 1: 輸入資料]
        FW_Orig["<b>原始 FW.bin (64KB)</b><br/>(內含 0xFF 預留空間)"]
        FW_Config_Example["<b>6-Byte 設定資訊 (範例)</b><br/>-----------------------------<br/>公鑰: <b>00</b> 90 a0<br/>驗證: <b>00</b> b0 c0<br/>-----------------------------<br/><b>解析步驟 (以 0x00 為例):</b><br/>1. 轉為二進位: <b>0b00000000</b><br/>2. 拆解位元:<br/>   - Bit 7  = <b>0</b> (有效)<br/>   - Bit 6  = <b>0</b> (啟用跟隨模式)<br/>   - Bit 0-5 = <b>0</b> (無效)"]
    end

    subgraph S2 [階段 2: 前置檢查]
        PreCheck{"Tool String / Bit7<br/>是否設定啟用？"}
    end

    subgraph S3 [階段 3: eToken Sign Code 處理]
        Tool("eToken 工具<br/>(計算 Hash, 覆寫資料)")
    end
    
    subgraph S4 [階段 4: 產生最終檔案]
        subgraph S4a [最終 .bin 檔]
           FW_Final_Bin["<b>FW_signed.bin</b><br/>(大小不變)<br/>(內部空間被覆寫)"]
        end
        subgraph S4b [最終 .rom 檔]
           FW_Final_Rom["<b>FW.rom</b><br/>(Key/Sig 與 FW 在同一 Block)"]
        end
    end
    ...
    %% Conditional branch
    PreCheck --"否"--> Abort((流程中止))

    %% --- Apply Styles ---
    class FW_Orig,FW_Final_Bin,FW_Final_Rom file
    class FW_Config_Example config
    class Tool process
    class PreCheck,Abort condition
```
### 流程說明與範例結合
...
flowchart TD
    ...
    classDef file fill:transparent,stroke:#6B7280,stroke-width:2px
    classDef process fill:transparent,stroke:#4338CA,stroke-width:2px
    classDef data fill:transparent,stroke:#F59E0B,stroke-width:2px
    classDef condition fill:transparent,stroke:#DC2626,stroke-width:2.5px
    classDef config fill:transparent,stroke:#10B981,stroke-width:2px,stroke-dasharray: 5 5

    %% --- Stages ---
    subgraph T1 [階段 1: 輸入資料]
        FW_Orig_1["<b>原始 FW.bin</b><br/>(內部無預留空間)"]
        PK_Data["<b>Public Key 資料</b>"]
        
        %% MODIFIED: Added detailed parsing steps inside this node.
        FW_Config_Example["<b>6-Byte 設定資訊 (範例)</b><br/>-----------------------------<br/>公鑰: <b>42</b> 00 10<br/>驗證: <b>42</b> 20 30<br/>-----------------------------<br/><b>解析步驟 (以 0x42 為例):</b><br/>1. 轉為二進位: <b>0b01000010</b><br/>2. 拆解位元:<br/>   - Bit 7  = <b>0</b> (有效)<br/>   - Bit 6  = <b>1</b> (啟用指定模式)<br/>   - Bit 0-5 = <b>2</b> (目標 Block = 2)"]
    end

    subgraph T2 [階段 2: 前置檢查]
        PreCheck_1{"Tool String / Bit7<br/>是否設定啟用？"}
    end

    subgraph T3 [階段 3: eToken Sign Code 處理]
        Tool_1("eToken 工具<br/>(計算 Hash, 組合檔案)")
    end
    
    subgraph T4 [階段 4: 產生最終檔案]
        subgraph T4a [最終 .bin 檔]
            FW_Final_Bin_1["<b>FW_signed.bin</b><br/>(大小變大)<br/>(資料被附加在後)"]
        end
        subgraph T4b [最終 .rom 檔]
            FW_Final_Rom_1["<b>FW.rom</b><br/>(Key/Sig 在 Block 2)"]
        end
    end
    ...
    %% Conditional branch
    PreCheck_1 --"否"--> Abort_1((流程中止))
    
    %% --- Apply Styles ---
    class FW_Orig_1,FW_Final_Bin_1,FW_Final_Rom_1 file
    class FW_Config_Example config
    class PK_Data data
    class Tool_1 process
    class PreCheck_1,Abort_1 condition
```
### 流程說明與範例結合
現在，我們將上圖的流程與您的具體配置範例結合起來說明：
假設 Hub FW bin config 配置如下:
