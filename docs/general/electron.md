...
graph TD
    subgraph Electron_App
        mainjs[main.js<br>Electron Main Process]
        preloadjs[preload.js<br>Preload Script]
        rendererjs[renderer.js<br>Renderer Process]
        html[index_2.html<br>UI畫面]
    end

    subgraph Native_Addon
        bindinggyp[binding.gyp<br>Build Config]
        addonn[addon.node<br>Build Output]
        addoncpp[addon.cpp<br>Windows N-API]
        addonmm[addon.mm<br>macOS N-API]
    end

    subgraph Native_Libraries
        dll[GlOciDll_Hub_L1.dll<br>Windows DLL]
        framework[GlOciDll_Hub_L1.framework<br>macOS Framework]
    end

    %% UI流程
    mainjs --> preloadjs
    mainjs -->|載入| html
    preloadjs --> rendererjs
    html --> rendererjs

    %% JS 與 native addon 的互動
    mainjs -->|require| addonn
    preloadjs -->|bridge| addonn
    rendererjs -->|呼叫 API| preloadjs

    %% binding.gyp 決定 build 哪個檔
    bindinggyp -->|Windows build| addoncpp
    bindinggyp -->|macOS build| addonmm

    addoncpp -->|動態載入| dll
    addonmm -->|NSBundle載入| framework

    %% build 後產出
    addoncpp -.build.-> addonn
    addonmm -.build.-> addonn

    %% 設定樣式
    classDef file fill:#fff,stroke:#222,stroke-width:1px;
    class mainjs,preloadjs,rendererjs,html,bindinggyp,addoncpp,addonmm,addonn,dll,framework file;
      ...
      return `
        <div class="chip-card flex items-center justify-between px-0 py-2">
          <div class="flex items-center">
            <div class="${iconClass}">
              <img src="${iconFiles[chip.name] || 'hub_icon_purple.svg'}" alt="${chip.name} Icon" class="icon-shadow" />
            </div>
            <div>
