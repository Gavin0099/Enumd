...
notion_updated_at: '2026-04-16T02:16:00.000Z'
exported_at: '2026-05-16T12:37:01.454Z'
is_summarized: false
relations:
  manual: []
  inferred:
...
- **Apple 的安全設計 (TCC)**
  ...
  自 macOS Mojave (10.14) 起，Apple 引入 **TCC (Transparency, Consent, and Control)**，規定所有應用若要存取敏感資料夾（桌面、文件、下載、外接磁碟等），必須由使用者親自授權。

- **企業挑戰**

  在數百台裝置上逐一手動勾選 FDA 權限，不僅效率低下，且無法保證設定正確完整，形成 **資安與管理缺口**。

- **Apple 官方解法**

  唯一能繞過手動授權的方法是透過 **MDM 部署 PPPC (Privacy Preferences Policy Control) Profile**。
...
1. **Apple 基礎服務 (生態系基礎)**
  ...
  - **ABM/ASM**：確保新裝置自動進入「受監督」模式
  
  - **APNs**：推播通知裝置，觸發與 MDM 連線

1. **受管控 macOS 裝置 (政策接收者)**

  - 接收 Profile → 更新 TCC.db
...
| SystemPolicyAllFiles | FDA 權限代號 | 固定值 |

| Identifier | 應用程式唯一識別碼 | `mdls -name kMDItemCFBundleIdentifier -r <app_path>` |

| IdentifierType | 識別碼類型 | 一般為 `bundleID` |

| CodeRequirement | 驗證簽章，避免偽冒 | `codesign -dr - <app_path>` |
...
- **驗證方式**：
  ...
  1. `profiles -P` → 確認 Profile 已安裝
  
  1. `sqlite3 ~/Library/Application\ Support/com.apple.TCC/TCC.db` → 查詢應用程式授權紀錄
  
  1. 啟動應用程式 → 應能直接存取 Documents/Downloads，無需人工點選授權
...
| **Profile 安裝確認** | `profiles -P` 或 MDM Console 查詢 | 每月抽樣 |

| **TCC 狀態檢查** | 查詢 TCC.db → 應有 `kTCCServiceSystemPolicyAllFiles` 記錄 | 每季度 |

| **App 簽名驗證** | `codesign -dr -` → 比對與 Profile 一致 | 每次大版本更新後 |

| **合規性報告** | 從 MDM 匯出合規性報表 (ProfileList, SecurityInfo) | 每月 |
...
1. 前往 [**business.apple.com**](https://www.google.com/url?sa=E&q=https%3A%2F%2Fbusiness.apple.com%2F) 進行註冊。
...
1. **關鍵設定**：
  ...
  - ✅ **強制監督 (Mandatory Supervision)**：必須勾選。
  
  - ❌ **允許移除 MDM (Allow Profile Removal)**：必須取消勾選。
  
  - ⚙️ **設定輔助程式略過選項**：勾選要為使用者跳過的初始設定步驟（如 Apple ID、Siri），以簡化流程。
...
1. 裝置自動聯繫 Apple，螢幕出現**無法跳過**的「遠端管理」畫面。
